...
function is31Flavors(arrayItem) {
    if (arrayItem.length == 31) {
        return "True";
    } else {
        return "False";
    }
...
function addFlavor(arrayItem, flavor) {
    arrayItem.unshift(flavor);
    /*code here*/
...
function removeLastFlavor(arrayItems) {
    arrayItems.pop();
...
function getFlavorByIndex(arrayName, Index) {
...
function removeFlavorByName(arrayName, flavor) {
    arrayName.splice(arrayName.indexOf(flavor), 1);
    return arrayName;
    /*code here*/
}
...
function copy(newArray, originalArray) {
    newArray = Array.from(originalArray);
    /*code here*/
    return newArray;
}
var newArray = copy(newArray, originalFlavors);
...
function filterByWord(itemArray, name) {
    let filteredArray = [];
    for (let index = 0; index < itemArray.length; index++) {
        if (itemArray[index].includes(name)) {
            filteredArray.push(itemArray[index]);
        }
    }
...
function getAverageWordLength(name) {
    let wordcount = 0;
    for (i = 0; i < name.length; i++) {
        wordcount += name[i].split(" ").length;
        console.log(wordcount);
    }
    return wordcount / name.length;
}
...
function getRandomFlavors(
    ...
    finalArray = originalFlavors.concat(
        newFlavors,
        seasonalFlavors,
        regionalFlavors
    );
